...
import "package:parish_connect/widgets/builds/build_stat_card.dart";
...
class SCCDetailView extends ConsumerWidget {
  ...
  pw.Widget _buildNarrativeSection(String title, List<String> contentList) {
    if (contentList.isEmpty ||
        contentList.every((item) => item.trim().isEmpty)) {
      return pw.Column(
  ...
  Future<void> _generateReportPdf(BuildContext context, WidgetRef ref) async {
            ...
            children: [
              ...
              pw.Table(
                ...
                children: [
                  _buildStatRow(
                    'Total of Families',
                    report.totalFamilies,
                    'Gospel Sharing Groups',
                    report.gospelSharingGroups,
                    'Council Meetings',
                    report.councilMeetings,
                  ),
                  _buildStatRow(
                    'Children',
                    report.children,
                    'Gospel Sharing Sessions Expected',
                    report.gospelSharingExpected,
                    'General Meetings',
                  ...
                  _buildStatRow(
                    'Youth',
                    report.youth,
                    'Gospel Sharing Sessions Done',
                    report.gospelSharingDone,
                    'No of Commissions',
                    report.noOfCommissions,
                  ),
                  _buildStatRow(
                    'Adults',
                    report.adults,
                    'No of Christians attending Gospel Sharing',
                    report.noChristiansAttendingGS,
                    'Active Commissions',
                    report.activeCommissions,
                  ),
                  _buildStatRow(
                    'Total Membership',
                    report.totalMembership,
                    '% Attendance of Gospel sharing',
                    '${(report.gsAttendancePercentage * 100).toStringAsFixed(0)}%',
                    'Baptism',
                    report.baptism,
                  ),
                  _buildStatRow(
                    'Lapsed Christians',
                    report.lapsedChristians,
                    'Irregular Marriages',
                    report.irregularMarriages,
                    'Burials',
                    report.burials,
                  ),
                ],
              ...
              _buildNarrativeSection(
                'Biblical Apostolate',
                report.biblicalApostolateActivities,
              ),
    ...
    final String path = '${extDirectory.path}/SCC_Reports';
    final Directory sccDir = Directory(path);
    ...
    final String fileName =
        'SCC_${report.sccName}_${DateTime.now().millisecondsSinceEpoch}.pdf';
    final File file = File('${sccDir.path}/$fileName');
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          buildExpansionSection(
            context,
            'Biblical Apostolate Activities',
            report.biblicalApostolateActivities,
          ),
          buildExpansionSection(
            context,
            'Liturgy Activities',
            report.liturgyActivities,
          ),
